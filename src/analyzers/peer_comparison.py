...
def analyze_peers(df: pd.DataFrame) -> list[Insight]:
    ...
    fleet_stats = (
        ...
        .reset_index()
    )  # turns the timestamp index back into a column to merge later

    merged = df.merge(fleet_stats, on="timestamp", how="left")  # join left like
    merged["hashrate_residual"] = (
        merged["hashrate_ths"] - merged["median_hashrate"]
    )  # compute diffs from median
    merged["temp_diff"] = (
        merged["chip_temp_c"] - merged["median_chip_temp"]
    )  # compute diffs from median
    ...
    anomaly_counts = (
        merged[merged["anomaly"]].groupby("miner_id").size().sort_values(ascending=False)
    )
    ...
    for miner_id, count in anomaly_counts.items():
        ...
        pct = count / total * 100
        mean_residual = merged.loc[
            (merged["miner_id"] == miner_id) & merged["anomaly"],
            "hashrate_residual",
        ].mean()
        severity = (
            "critical"
            if pct > PEER_CRITICAL_PCT
            else "warning"
            if pct > PEER_WARNING_PCT
            else "info"
        )
        insights.append(
            {
                ...
                "action": ("Investigate ASIC board health and firmware version for this miner."),
            }
        )
        ...
        insights.append(
            {
                ...
                "severity": "info",
                "detail": (f"Top anomaly frequency ranking (% of readings flagged): {ranking}"),
                "metric": len(ranking),
